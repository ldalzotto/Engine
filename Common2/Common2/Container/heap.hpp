...
namespace v2
{
	...
	struct HeapA
	{
		...
		template<class ShadowHeap_t(s)>
		static AllocationState allocate_element_with_alignment(ShadowHeap_t(s)& p_heap,
			const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk);

		template<class ShadowHeap_t(s)>
		static AllocationState allocate_element_norealloc_with_alignment(ShadowHeap_t(s)& p_heap,
			const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk);
	...
	private:
		...
		template<class ShadowHeap_t(s)>
		static int8 _allocate_element_with_alignment(ShadowHeap_t(s)& p_heap, const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk);
	...
	*/
	struct Heap
	{
		...
		HeapA::AllocationState allocate_element_with_alignment(const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk);
		HeapA::AllocationState allocate_element_norealloc_with_alignment(const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk);
	...
	*/
	struct HeapPaged
	{
		...
		*/
		...
		AllocationState allocate_element_norealloc_with_alignment(const uimax p_size, const uimax p_alignement_modulo, AllocatedElementReturn* out_chunk);
...
namespace v2
{
	...
	inline HeapA::AllocationState Heap::allocate_element_with_alignment(const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk)
	{
		return HeapA::allocate_element_with_alignment(*this, p_size, p_alignement_modulo, out_chunk);
	};

	inline HeapA::AllocationState Heap::allocate_element_norealloc_with_alignment(const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk)
	{
		return HeapA::allocate_element_norealloc_with_alignment(*this, p_size, p_alignement_modulo, out_chunk);
	};
	...
	inline HeapPaged::AllocationState HeapPaged::allocate_element_norealloc_with_alignment(const uimax p_size, const uimax p_alignement_modulo, AllocatedElementReturn* out_chunk)
	{
	...
	template<class ShadowHeap_t(s)>
	inline HeapA::AllocationState HeapA::allocate_element_with_alignment(ShadowHeap_t(s)& p_heap, const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk)
	{
		if (!_allocate_element_with_alignment(p_heap, p_size, p_alignement_modulo, out_chunk))
		{
			_defragment(p_heap);
			if (!_allocate_element_with_alignment(p_heap, p_size, p_alignement_modulo, out_chunk))
			{
				uimax l_heap_size = p_heap.sh_get_size();
				p_heap.sh_resize(l_heap_size == 0 ? (p_alignement_modulo > p_size ? p_alignement_modulo : p_size) : ((l_heap_size * 2) + (p_alignement_modulo > p_size ? p_alignement_modulo : p_size)));

#if CONTAINER_MEMORY_TEST
				assert_true(
#endif
					_allocate_element_with_alignment(p_heap, p_size, p_alignement_modulo, out_chunk)
#if CONTAINER_MEMORY_TEST
				)
#endif
					;
	...
	template<class ShadowHeap_t(s)>
	inline HeapA::AllocationState HeapA::allocate_element_norealloc_with_alignment(ShadowHeap_t(s)& p_heap, const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk)
	{
		if (!_allocate_element_with_alignment(p_heap, p_size, p_alignement_modulo, out_chunk))
		{
			_defragment(p_heap);
	...
	template<class ShadowHeap_t(s)>
	inline int8 HeapA::_allocate_element_with_alignment(ShadowHeap_t(s)& p_heap, const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk)
	{
#if CONTAINER_BOUND_TEST
		assert_true(p_size != 0);
#endif
