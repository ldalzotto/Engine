...
namespace v2
{
	...
	*/
	...
	inline void D3RendererHeap::remove_shader(const Token(ShaderIndex) p_shader)
	{
		...
		for (loop_reverse(j, 0, this->shaders_indexed.Size))
		{
			if (tk_eq(this->shaders_indexed.get(j), p_shader))
			{
				this->shaders_indexed.erase_element_at(j);
				break;
			}
		}
		...
		for (loop(i, 0, this->heap.shaders_indexed.Size))
		{
			...
			for (loop(j, 0, l_materials.get_size()))
			{
				...
				for (loop(k, 0, l_renderable_objects.get_size()))
				{
					...
					p_graphics_binder.bind_vertex_buffer_gpu(p_graphics_binder.buffer_allocator.get_buffergpu(l_renderable_object.mesh.gpu_memory));
					p_graphics_binder.draw(l_renderable_object.mesh.vertex_count);
	...
	inline Token(RenderableObject) D3Renderer::allocate_renderable_object(BufferAllocator& p_buffer_allocator, GraphicsAllocator& p_graphics_allocator, const Slice<Vertex>& p_initial_vertices)
	{
		...
		l_renderable_object.mesh = Mesh{ p_buffer_allocator.allocate_buffergpu(sizeof(Vertex) * p_initial_vertices.Size, (BufferUsageFlag)((BufferUsageFlags)BufferUsageFlag::VERTEX | (BufferUsageFlags)BufferUsageFlag::TRANSFER_WRITE)),
										 p_initial_vertices.Size };
		p_buffer_allocator.write_to_buffergpu(l_renderable_object.mesh.gpu_memory, p_initial_vertices.build_asint8());
		...
		return this->heap.allocate_renderable_object(l_renderable_object);
	};
