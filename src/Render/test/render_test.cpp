...
inline void bufferstep_test()
{
    ...
    RenderableObject_Token l_renderable_object =
    ...
    RenderableObject_Token l_renderable_object2 =
        D3RendererAllocatorComposition::allocate_renderable_object_with_mesh_and_buffers(l_ctx.buffer_memory, l_ctx.graphics_allocator, l_d3_renderer.allocator, l_test_vertices, l_test_indices);
...
inline void shader_linkto_material_allocation_test()
{
...

    {
        ...
        ShaderIndex l_shader_index_executed_after = {5, token_build_default<Shader_TokenValue>(), token_build_default<ShaderLayout_TokenValue>()};
        ShaderIndex_Token l_shader_index_executed_after_token = l_d3_renderer.allocator.allocate_shader(l_shader_index_executed_after);

        assert_true(l_d3_renderer.allocator.heap.shaders_to_materials.Memory.varying_vector.get_size() == 1);
        assert_true(l_d3_renderer.allocator.heap.shaders_to_materials.get_vector(token_build_from<PoolOfVector<Material_Token>::sTokenValue>(l_shader_index_executed_after_token)).Size == 0);

        ShaderIndex l_shader_index_executed_before = {1, token_build_default<Shader_TokenValue>(), token_build_default<ShaderLayout_TokenValue>()};
        ShaderIndex_Token l_shader_index_executed_before_token = l_d3_renderer.allocator.allocate_shader(l_shader_index_executed_before);

        assert_true(l_d3_renderer.allocator.heap.shaders_to_materials.Memory.varying_vector.get_size() == 2);
        assert_true(l_d3_renderer.allocator.heap.shaders_to_materials.get_vector(token_build_from<PoolOfVector<Material_Token>::sTokenValue>(l_shader_index_executed_after_token)).Size == 0);
        assert_true(l_d3_renderer.allocator.heap.shaders_to_materials.get_vector(token_build_from<PoolOfVector<Material_Token>::sTokenValue>(l_shader_index_executed_before_token)).Size == 0);
...
inline void draw_test()
{
    ...
    ShaderModule_Token l_vertex_shader_module = l_ctx.graphics_allocator.allocate_shader_module(l_vertex_shader_compiled.get_compiled_binary());
    ShaderModule_Token l_fragment_shader_module = l_ctx.graphics_allocator.allocate_shader_module(l_fragment_shader_compiled.get_compiled_binary());

    SliceN<ShaderLayoutParameterType, 1> l_shader_layout_arr{ShaderLayoutParameterType::UNIFORM_BUFFER_VERTEX_FRAGMENT};
    ShaderIndex_Token l_shader = D3RendererAllocatorComposition::allocate_colorstep_shader_with_shaderlayout(
    ...
    Material_Token l_red_material_token = l_d3_renderer.allocator.allocate_material(l_red_material);
    Material_Token l_green_material_token = l_d3_renderer.allocator.allocate_material(l_green_material);
    ...
    RenderableObject_Token l_obj_1, l_obj_2, l_obj_3, l_obj_4;
    ...
    BufferHost_Token l_color_attachment_token = GraphicsPassReader::read_graphics_pass_attachment_to_bufferhost(l_ctx.buffer_memory, l_ctx.graphics_allocator,
