...
struct Renderer_3D_imgui
{
    RenderTargetInternal_Color_Depth render_targets;
    D3Renderer d3_renderer;
    ImguiRenderer imgui_renderer;

    inline static Renderer_3D_imgui allocate(GPUContext& p_gpu_context, const RenderTargetInternal_Color_Depth::AllocateInfo& p_render_targets_allocate_info)
    {
        Renderer_3D_imgui l_return;
        l_return.render_targets = RenderTargetInternal_Color_Depth::allocate(p_gpu_context, p_render_targets_allocate_info);
        l_return.d3_renderer = D3Renderer::allocate(p_gpu_context, l_return.render_targets);
        l_return.imgui_renderer = ImguiRenderer::allocate(p_gpu_context, l_return.render_targets, RenderPassAttachment::ClearOp::NOT_CLEARED);
        return l_return;
    };
    ...
    {
        this->d3_renderer.free(p_gpu_context);
        this->imgui_renderer.free(p_gpu_context);
        this->render_targets.free(p_gpu_context);
    };
    ...
    {
        this->d3_renderer.buffer_step(p_gpu_context, p_totaltime);
        this->imgui_renderer.buffer_step(p_gpu_context.buffer_memory);
    };
