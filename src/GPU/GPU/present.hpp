...
struct GPUPresentDevice
{
    ...
    inline static GPUPresentDevice allocate(const GPUInstance& p_instance, const window_native p_window_handle)
    {
        ...
        l_present_device.device = (gc_t)token_value(p_instance.logical_device);
        l_present_device.surface = (gcsurface_t)token_value(gpu_create_surface(p_instance.instance, p_window_handle));
        ...
        vkGetPhysicalDeviceProperties((VkPhysicalDevice)token_value(p_instance.graphics_card.device), &l_physical_device_properties);

        uint32 l_present_queue_family_index = -1;
        Span<VkQueueFamilyProperties> l_queueFamilies = vk::getPhysicalDeviceQueueFamilyProperties((VkPhysicalDevice)token_value(p_instance.graphics_card.device));

        for (loop(j, 0, l_queueFamilies.Capacity))
        {
            uint32 l_is_supported = 0;
            vk_handle_result(vkGetPhysicalDeviceSurfaceSupportKHR((VkPhysicalDevice)token_value(p_instance.graphics_card.device), (uint32)j, l_present_device.surface, &l_is_supported));
            if (l_is_supported)
            {
        ...
        Span<VkSurfaceFormatKHR> l_surface_formats = vk::getPhysicalDeviceSurfaceFormatsKHR((VkPhysicalDevice)token_value(p_instance.graphics_card.device), l_present_device.surface);
    ...
    {
        vkDestroySurfaceKHR((VkInstance)token_value(p_instance.instance), this->surface, NULL);
    };

    inline void recalculate_surface_capabilities()
    {
        vk_handle_result(vkGetPhysicalDeviceSurfaceCapabilitiesKHR((VkPhysicalDevice)token_value(this->graphics_card.device), this->surface, &this->surface_capacilities));
    };
};
...
struct GPUPresent_2DQuad
{
    ...
    BufferGPU_Token d2_quad_vertices;
    BufferGPU_Token d2_quad_indices_image_indices;
...
struct SwapChain
{
    ...
    Span<ImageGPU_Token> swap_chain_images;
    Span<GraphicsPass_Token> rendertarget_copy_pass;
    Span<Shader_Token> image_copy_shaders;

    ShaderLayout_Token image_copy_shader_layout;
    ShaderModule_Token image_copy_shader_vertex;
    ShaderModule_Token image_copy_shader_fragment;
    ShaderTextureGPUParameter_Token shader_parameter_texture;

    inline static SwapChain allocate(GPUPresentDevice& p_present_device, BufferMemory& p_buffer_memory, GraphicsAllocator2& p_graphics_allocator, const TextureGPU_Token p_presented_texture,
  ...
  private:
    inline static void allocate_swap_chain_texture_independant(SwapChain& p_swap_chain, GPUPresentDevice& p_present_device, GraphicsAllocator2& p_graphics_allocator,
                                                               const TextureGPU_Token p_presented_texture, const Slice<int8>& p_compiled_vertex_shader, const Slice<int8>& p_compiled_fragment_shader)
    {
                                                                ...
                                                                const v3ui p_window_handle_dimensions)
    {
        ...
        p_swap_chain.swap_chain_images = Span<ImageGPU_Token>::allocate(l_images.Capacity);
        p_swap_chain.rendertarget_copy_pass = Span<GraphicsPass_Token>::allocate(l_images.Capacity);
        p_swap_chain.image_copy_shaders = Span<Shader_Token>::allocate(l_images.Capacity);
        ...
        for (loop(i, 0, l_images.Capacity))
        {
            ...
            SliceN<ImageGPU_Token , 1> l_attachment_images = {p_swap_chain.swap_chain_images.get(i)};
...
struct GPUPresent
{
    ...
    inline static GPUPresent allocate(const GPUInstance& p_gpu_instance, BufferMemory& p_buffer_memory, GraphicsAllocator2& p_graphics_allocator, const window_native p_window_handle,
                                      const v3ui p_window_size, TextureGPU_Token p_presented_texture, const Slice<int8>& p_compiled_vertex_shader, const Slice<int8>& p_compiled_fragment_shader)
    {
    ...
    inline void graphics_step(GraphicsBinder& p_graphics_binder)
    {
        ...
        vk_handle_result(vkAcquireNextImageKHR(this->device.device, this->swap_chain.swap_chain, 0, (VkSemaphore)token_value(this->swap_chain.swap_chain_next_image_semaphore.semaphore), VK_NULL_HANDLE,
