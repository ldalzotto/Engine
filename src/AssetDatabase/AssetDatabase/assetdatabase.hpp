...
static const int8* DB_ASSET_RESSOURCE_TABLE_INITIALIZATION = MULTILINE(create table if not exists asset_dependencies(id integer PRIMARY KEY, dependencies blob););
    ...
    inline static AssetDatabase allocate(DatabaseConnection& p_database_connection)
    {
        ...
        l_asset_database.asset_dependencies_count_query = SQLitePreparedQuery::allocate(p_database_connection, Slice_int8_build_rawstr(AssetDatabase_Const::ASSET_DEPENDENCIES_COUNT_SELECT_QUERY),
                                                                                        SQLiteQueryLayout::allocate_span(Span_allocate_slice(&tmp_layout_int64_slice)),
                                                                                        SQLiteQueryLayout::allocate_span(Span_allocate_slice(&tmp_layout_int64_slice)));

        l_asset_database.asset_dependencies_blob_select_query = SQLitePreparedQuery::allocate(p_database_connection, Slice_int8_build_rawstr(AssetDatabase_Const::ASSET_DEPENDENCIES_BLOB_SELECT_QUERY),
                                                                                              SQLiteQueryLayout::allocate_span(Span_allocate_slice(&tmp_layout_int64_slice)),
                                                                                              SQLiteQueryLayout::allocate_span(Span_allocate_slice(&tmp_layout_blob_slice)));
        ...
        l_asset_database.asset_dependencies_update_query =
            SQLitePreparedQuery::allocate(p_database_connection, Slice_int8_build_rawstr(AssetDatabase_Const::ASSET_DEPENDENCIES_BLOB_UPDATE_QUERY),
                                          SQLiteQueryLayout::allocate_span(Span_allocate_slice(&tmp_layout_blob_int64_slice)), SQLiteQueryLayout::build_default());
