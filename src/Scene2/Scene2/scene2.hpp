...
{
    for (loop(i, 0, this->component_removed_events.Size))
    {
...
inline void Scene::remove_node(const NodeEntry& p_node)
{
    ...
    this->tree.node_tree.traverse3(token_build_from(NTreeNode, p_node.Node->index), [this](const NodeEntry& p_tree_node) {
        ...
        {
            this->component_removed_events.push_back_element(ComponentRemovedEvent::build(token_build_from(Node, p_tree_node.Node->index), *Slice_get(&l_node_component_tokens, i)));
        }
...
inline NodeComponent* Scene::get_node_component_by_type(const Token(Node) p_node, const component_t p_type)
{
    ...
    {
        NodeComponent& l_component = *Slice_get(&l_components, i);
...
{
    for (loop(i, 0, this->orphan_nodes.Size))
    {
