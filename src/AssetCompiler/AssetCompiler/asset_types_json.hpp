...
struct ShaderAssetJSON
{
    ...
    inline static void push_dependencies_from_json_to_buffer(JSONDeserializer* p_json_deserializer, Vector<int8>* in_out_buffer)
    {
        ...
        l_shader_dependencies_value.push_to_binary_buffer(iVector<Vector<int8>>{*in_out_buffer});
    };
...
struct MaterialAssetJSON
{
    ...
    inline static MaterialResource::AssetDependencies allocate_dependencies_from_json(JSONDeserializer* p_json_deserializer, const Slice<int8>& p_root_path)
    {
        ...
        p_json_deserializer->next_field("shader");
        BinarySerializer::type(iVector<Vector<int8>>{l_binary}, HashFunctions::hash(p_json_deserializer->get_currentfield().value));

        {
            ...
            JSONDeserializer l_shader_asset_deserializer = JSONDeserializer::sanitize_and_start(iVector<Vector<int8>>{l_shader_file_content_vector});
        ...
        BinarySerializer::slice(iVector<Vector<int8>>{l_binary}, l_textures.Memory.slice.build_asint8());
