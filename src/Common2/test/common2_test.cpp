...
inline void vector_test()
{
    ...
    iVector<Vector<uimax>> l_vector_sizet_sahdow = iVector<Vector<uimax>>{l_vector_sizet};
...
inline void vector_slice_test()
{
    ...
    iVector<VectorSlice<uimax>> l_vector_sizet_shadow = iVector<VectorSlice<uimax>>{l_vector_sizet};
...
inline void varyingvector_test()
{
    ...
    // varyingvector_element_expand_with_value varyingvector_element_shrink
    {
        ...
        l_varyingvector.element_shrink_delta(2, sizeof(uimax));
    ...
    // element_expand
    {
        ...
        l_varyingvector.push_back(Slice<uimax>::build_asint8_memory_singleelement(&l_element_0));
        l_varyingvector.push_back(Slice<uimax>::build_asint8_memory_singleelement(&l_element_1));

        uimax l_first_index = l_varyingvector.get_size() - 2;
        uimax l_second_index = l_first_index + 1;

        assert_true(l_varyingvector.get_element_typed<uimax>(l_first_index).get(0) == l_element_0);
        assert_true(l_varyingvector.get_element_typed<uimax>(l_second_index).get(0) == l_element_1);

        l_varyingvector.element_expand(l_first_index, l_varyingvector.memory.get_capacity() * 2);
    ...
    // varyingvector_element_writeto
    {
        ...
        l_varyingvector.element_expand_delta(2, sizeof(uimax) * 3);
...
inline void vectorofvector_test()
{
    ...
    // vectorofvector_element_push_back_element
    {
        ...
        for (loop(i, 0, 2))
        {
            ...
            Slice<uimax> l_element_nested = l_entry.to_slice();
            assert_true(l_entry.get_size() == 1);
            assert_true(l_entry.get(0) == l_element);
    ...
    // vectorofvector_element_push_back_array
    {
        ...
        uimax l_old_size = l_entry.get_size();

        l_entry.push_back_array(l_elements_slice);

        {
            Slice<uimax> l_vector_element = l_entry.to_slice();
        ...
        l_entry.push_back_array(l_elements_slice);
        {
            ...
            assert_true(l_entry.get_capacity() == 8); //  l_vectorofvector_uimax.get_vectorheader(l_index)->Capacity
...

    {
...

        {
            ...
            assert_true(l_entry.get(3) == 9);
            assert_true(l_entry.get(2) == l_initial_elements_0[2]);
        }
        {
            assert_true(l_vectorofvector_uimax.get(1).get(0) == l_initial_elements_1[0]);
            assert_true(l_vectorofvector_uimax.get(1).get(1) == l_initial_elements_1[1]);
...
#if 1
    // iVector test on random elements
    {
        l_vectorofvector_uimax.free();
        l_vectorofvector_uimax = VectorOfVector<uimax>::allocate_default();
        for (loop(i, 0, 10))
...
inline void deserialize_json_test()
{
...
#endif
    {
        ...
        iVector<Vector<int8>> l_json_str_ivector = iVector<Vector<int8>>{l_json_str.Memory};
...
inline void serialize_json_test()
{
    ...
    iVector<Vector<int8>> l_compared_json_ivector = iVector<Vector<int8>>{l_compared_json.Memory};
...
inline void serialize_deserialize_binary_test()
{
    ...
    BinarySerializer::slice(iVector<Vector<int8>>{l_binary_data}, l_slice.build_asint8());
    ...
    BinarySerializer::slice(iVector<VectorSlice<int8>>{l_binary_data_slice}, slice_from_slicen(&l_slice_arr).build_asint8());
...
inline void command_buffer_pattern_test()
{
    ...
    using namespace CommandBuffer_Types;
    ...
    CommandBuffer_Token<MathOpCommand> l_add_command_buffer_token = l_command_pool.allocate_command_buffer();
    ...
    CommandBuffer_Token<MathOpCommand> l_mul_command_buffer_token = l_command_pool.allocate_command_buffer();
    ...
    CommandBufferExecution_Token<MathOpCommand> l_add_command_execution = l_command_execution.push_command_buffer(l_add_command_buffer_token);
    ...
    uimax l_result = 2;
    l_command_execution.process_command_buffer_tree(l_command_pool, [&](const CommandBuffer_Token<MathOpCommand>, CommandBuffer<MathOpCommand>& p_command) {
...
namespace Common2HeapTypes
{
using Elements = Pool<Element>;
using Element_Token = Elements::sToken;
}; // namespace Common2HeapTypes
...
using namespace Common2HeapTypes;

struct HHeap
{
    Elements element;

    inline Element& get_element(const Element_Token p_token)
    {
        return this->element.get(p_token);
    };
};
