...
struct SQLitePreparedQuery
{
	...
	inline static SQLitePreparedQuery allocate(DatabaseConnection& p_connection, const Slice<int8>& p_query, const SQLiteQueryLayout& p_parameter_layout, const SQLiteQueryLayout& p_return_layout)
	{
		...
		DatabaseConnection_Utils::handleSQLiteError(sqlite3_prepare_v3(p_connection.connection, p_query.Begin, (int32)p_query.Size, SQLITE_PREPARE_PERSISTENT, &l_statement, NULL), &p_connection.connection);
#if MEM_LEAK_DETECTION
		push_ptr_to_tracked((int8*)l_statement);
#endif
		return SQLitePreparedQuery{
				p_parameter_layout,
				p_return_layout,
				l_statement
		};
...
struct SQliteQueryExecution
{
	...
	inline static void execute_sync(DatabaseConnection& p_connection, sqlite3_stmt* p_statement, const ForeachRowFunc_t& p_foreach_row)
	{
