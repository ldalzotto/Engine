...
mutex_native mutex_native_allocate(){
    ...
    l_mutex.ptr = (int8*)CreateMutex(NULL, 0, NULL);
#if __DEBUG
    assert_true(l_mutex.memory != NULL);
#endif
    return l_mutex;
};
void mutex_native_lock(const mutex_native& p_mutex){
    DWORD l_wait = WaitForSingleObject((HANDLE)p_mutex.ptr, INFINITE);
#if __DEBUG
...
void mutex_native_unlock(const mutex_native& p_mutex){
    BOOL l_rm = ReleaseMutex((HANDLE)p_mutex.ptr);
#if __DEBUG
    assert_true(l_rm);
#endif
};
...
void mutex_native_free(mutex_native& p_mutex){
    BOOL l_ch = CloseHandle((HANDLE)p_mutex.ptr);
#if __DEBUG
    assert_true(l_ch);
#endif
    p_mutex.ptr = 0;
