...
template <class ElementType> using PoolFreeBlocks_t = Vector<TokenT(ElementType)>;
...
template <class ElementType> struct Pool
{
    ...
    {
        return Pool<ElementType>{Vector<ElementType>::allocate(p_memory_capacity), Vector<TokenT(ElementType)>::build_zero_size(cast(TokenT(ElementType)*, NULL), 0)};
    };
    ...
    inline int8 is_element_free(const TokenT(ElementType) p_token)
    {
    ...
    inline ElementType& get(const TokenT(ElementType) p_token)
    {
    ...
    inline TokenT(ElementType) alloc_element_empty()
    {
        if (!this->free_blocks.empty())
        {
            TokenT(ElementType) l_availble_token = this->free_blocks.get(this->free_blocks.Size - 1);
        ...
        else
        {
            ...
            return TokenT(ElementType){this->memory.Size - 1};
        }
    }
    ...
    inline void release_element(const TokenT(ElementType) p_token)
    {
#if CONTAINER_BOUND_TEST
  ...
  private:
    inline void element_free_check(const TokenT(ElementType) p_token)
    {
#if CONTAINER_BOUND_TEST
    ...
    inline void element_not_free_check(const TokenT(ElementType) p_token)
    {
#if CONTAINER_BOUND_TEST
        if (this->is_element_free(p_token))
        {
