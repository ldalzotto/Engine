...
struct HeapA
{
    ...
    inline static AllocationState allocate_element_with_modulo_offset(ShadowHeap(s) & p_heap, const uimax p_size, const uimax p_modulo_offset, HeapA::AllocatedElementReturn* out_chunk)
    {
    ...
    template <class ShadowHeap(s)>
    inline static AllocationState allocate_element_norealloc_with_modulo_offset(ShadowHeap(s) & p_heap, const uimax p_size, const uimax p_alignement_modulo, HeapA::AllocatedElementReturn* out_chunk)
    {
  ...
  private:
#if __DEBUG
    inline static void _assert_memory_alignment(const uimax p_alignment_modulo, const HeapA::AllocatedElementReturn& p_allocated_chunk)
    {
        assert_true((p_allocated_chunk.Offset % p_alignment_modulo) == 0);
    };
#endif

    template <class ShadowHeap(s)> inline static int8 _allocate_element(ShadowHeap(s) & p_heap, const uimax p_size, HeapA::AllocatedElementReturn* out_return)
    {
#if __DEBUG
    ...
    template <class ShadowHeap(s)>
    inline static int8 _allocate_element_with_modulo_offset(ShadowHeap(s) & p_heap, const uimax p_size, const uimax p_modulo_offset, HeapA::AllocatedElementReturn* out_chunk)
    {
#if __DEBUG
        assert_true(p_size != 0);
#endif
    ...
    template <class ShadowHeap(s)> inline static AllocatedElementReturn _push_chunk(ShadowHeap(s) & p_heap, SliceIndex* p_chunk)
    {
        return HeapA::AllocatedElementReturn::build(sh_c_get_allocated_chunks(p_heap).alloc_element(*p_chunk), p_chunk->Begin);
    };

    template <class ShadowHeap(s)> inline static void _defragment(ShadowHeap(s) & p_heap)
    {
        using ShadowVector(free_chunks) = decltype(sh_c_get_freechunks(p_heap));
        ShadowVector(free_chunks)& l_free_chunks = sh_c_get_freechunks(p_heap);
...
struct Heap
{
    ...
    inline HeapA::AllocationState allocate_element(const uimax p_size, HeapA::AllocatedElementReturn* out_chunk)
    {
        return HeapA::allocate_element(*this, p_size, out_chunk);
    };

    inline HeapA::AllocationState allocate_element_with_modulo_offset(const uimax p_size, const uimax p_modulo_offset, HeapA::AllocatedElementReturn* out_chunk)
    {
        return HeapA::allocate_element_with_modulo_offset(*this, p_size, p_modulo_offset, out_chunk);
    };

    inline HeapA::AllocationState allocate_element_norealloc_with_modulo_offset(const uimax p_size, const uimax p_modulo_offset, HeapA::AllocatedElementReturn* out_chunk)
    {
