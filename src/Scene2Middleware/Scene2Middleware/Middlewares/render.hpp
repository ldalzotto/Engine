...
struct RenderHeap
{
    ...
    {
        return RenderHeap{PoolHashedCounted<hash_t, ShaderModuleRessource>::allocate_default(), PoolHashedCounted<hash_t, MeshRessource>::allocate_default(),
...
struct RenderRessourceAllocator2
{
    ...
    {
        return RenderRessourceAllocator2{RenderHeap::allocate(),
    ...
    inline void step(D3Renderer& p_renderer, GPUContext& p_gpu_context, AssetDatabase& p_asset_database)
    {
        ...
        for (loop_reverse(i, 0, this->mesh_renderer_allocation_events.Size))
        {
            ...
            l_mesh_renderer.renderable_object = D3RendererAllocatorComposition::allocate_renderable_object_with_buffers(
                p_gpu_context.buffer_memory, p_gpu_context.graphics_allocator, p_renderer.allocator, this->heap.mesh_v2.pool.get(l_mesh_renderer.dependencies.mesh).mesh);
            p_renderer.allocator.heap.link_material_with_renderable_object(this->heap.materials.pool.get(l_mesh_renderer.dependencies.material).material, l_mesh_renderer.renderable_object);
...
struct RenderRessourceAllocator2Composition
{
    ...
    inline static Token(MeshRendererComponent)
        allocate_meshrenderer_inline_with_dependencies(RenderRessourceAllocator2& p_render_ressource_allocator, const ShaderModuleRessource::InlineAllocationInput& p_vertex_shader,
                                                       const ShaderModuleRessource::InlineAllocationInput& p_fragment_shader, const ShaderRessource::InlineAllocationInput& p_shader,
                                                       const MaterialRessource::InlineAllocationInput& p_material, const MeshRessource::InlineAllocationInput& p_mesh, const Token(Node) p_scene_node)
    {
    ...
    inline static Token(MeshRendererComponent)
        allocate_meshrenderer_database_with_dependencies(RenderRessourceAllocator2& p_render_ressource_allocator, const ShaderModuleRessource::DatabaseAllocationInput& p_vertex_shader,
                                                         const ShaderModuleRessource::DatabaseAllocationInput& p_fragment_shader, const ShaderRessource::DatabaseAllocationInput& p_shader,
                                                         const MaterialRessource::DatabaseAllocationInput& p_material, const MeshRessource::DatabaseAllocationInput& p_mesh,
...
struct RenderMiddleWare
{
    ...
    {
        return RenderMiddleWare{RenderRessourceAllocator2::allocate()};
    };
    ...
    inline void step(D3Renderer& p_renderer, GPUContext& p_gpu_context, AssetDatabase& p_asset_database, Scene* p_scene)
    {
        ...
        if (this->allocator.camera_component.allocated)
        {
            ...
            {
                p_renderer.color_step.set_camera_projection(p_gpu_context, this->allocator.camera_component.asset.Near, this->allocator.camera_component.asset.Far,
                                                            this->allocator.camera_component.asset.Fov);
